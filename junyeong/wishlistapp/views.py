...
def mypost(request):
    ...
    return render(request, "mypost.html", {"blogs": blogs})
...
def detail(request, id):
    ...
    if request.user.is_authenticated:
        already_in_wishlist = Wishlist.objects.filter(
            user=request.user, post=blog
        ).exists()

    return render(
        request,
        "detail.html",
        {"blog": blog, "already_in_wishlist": already_in_wishlist},
    )

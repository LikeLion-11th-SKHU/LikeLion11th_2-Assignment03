...
from .models import Blog, Wishlist, Mywriting
...
def add_to_wishlist(request, post_id):
    ...
    if not already_in_wishlist:
        wishlist_item, created = Wishlist.objects.create(user=request.user, post=post)
...
def mywriting(request):
    user = request.user
    mywriting = Blog.objects.filter(user=user)

    return render(request, 'mywriting.html', {'mywriting':mywriting})